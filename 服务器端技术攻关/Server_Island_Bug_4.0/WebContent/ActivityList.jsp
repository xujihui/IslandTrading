		...
		<table class="imgtable">
			<thead>
				<tr>
					...
					<th width="100px;">活动内容</th>
					<th width="100px;">活动组织者</th>
					<th width="100px;">活动发布时间</th>
					<th width="100px;">活动发布位置</th>
					<th width="100px;">活动详情图</th>
					<th width="100px;" colspan="2">操作</th>
				</tr>
			</thead>
			<tbody>
				<c:forEach items="${proactivity}" var="pro">
					<tr>
						<td>${pro.Activity_Id}</td>
						<td>${pro.Activity_Name}</td>
						<td>${pro.Activity_Content}</td>
						<td>${pro.Activity_Organizer}</td>
						<td>${pro.Activity_Time}</td>
						<td>${pro.Activity_Site}</td>
						<td>${pro.Activity_Img}</td>
						<td><a href="${ctx}/activity/deleteByID/${pro.Activity_Id}">删除</a></td>
						<td><a
							href="${ctx}/editActivity.jsp?ACTIVITY_ID=${pro.Activity_Id}
							&ACTIVITY_NAME=${pro.Activity_Name}&ACTIVITY_CONTENT=${pro.Activity_Content}
							&ACTIVITY_ORGANIZER=${pro.Activity_Organizer}&ACTIVITY_TIME=${pro.Activity_Time}
							&ACTIVITY_SITE=${pro.Activity_Site}&ACTIVITY_IMG=${pro.Activity_Img}">编辑</a></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
