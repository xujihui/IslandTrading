...
public class ProductController extends Controller {
	...
	public void save() {
		...
		Date pTime = (Date) this.getParaToDate("PRODUCT_TIME");
		...
		if ((pID != null && pID.trim() != "") && (pName != null && pName.trim() != "")) {
			...
			if (rec == null) {
				boolean res = productService.save(pID, pName, pPrice, pDescribe, pImage, pTime, pSite, pView, pPositive,
						pStatus, pMessage);
				if (res) {
					result = "商品添加成功,继续添加";
				} else {
	...
	public void update() {
		...
		// float pPrice = Float.parseFloat(this.getPara("PRODUCT_PRICE"));
		String pDescribe = this.getPara("PRODUCT_DESCRIBE");
		String pImage = this.getPara("PRODUCT_IMAGE");
