...
<body style="background: #f0f9fd;">
	...
	<div class="center">
		<form action="${ctx}/user/findByID" method="post">
			用户ID：<input type="text" name="User_Id" size="40" id="User_Id" /> <input
				type="submit" style="background: #DDDDFF" value="查找"><a
				href="${ctx}/user/list" target="rightFrame"><u>返回用户列表页</u></a>
		</form>
				...
				<table class="imgtable">
					...
					<tbody>
						...
						<tr>
							...
							<td>${user.User_Power}</td>
							<td>${user.User_TakingId}</td>
							<td>${user.User_Tel}</td>
							<td>${user.Hx_Username}</td>
							<td>${user.Hx_Password}</td>
						</tr>
					</tbody>
				</table>
			</c:if>
