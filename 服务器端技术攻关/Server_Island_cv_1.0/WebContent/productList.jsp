			...
			<tbody>
				<c:forEach items="${prolist}" var="pro">
					<tr>
						...
						<td>${pro.price}</td>
						<td>${pro.image}</td>
						<td>${pro.t_describe}</td>
						<td>${pro.classify_id}</td>
						<td>${pro.site}</td>
						<td>${pro.time}</td>
						<td><a href="${ctx}/product/deleteByID/${pro.id}">删除</a></td>
						<td><a
							href="${ctx}/editProduct.jsp?proID=${pro.id}
							&proName=${pro.name}&proPrice=${pro.price}&proImage=${pro.image}
							&proT_describe=${pro.t_describe}&proClassify=${pro.classify_id}
							&proSite=${pro.site}&proTime=${pro.time}">编辑</a></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
