...
//				writer.write("没有指定商品   Product_Id:" + pid);
	 ...
	 * 触发方式：http://localhost:8080/IslandTrading/analysis/type_collection?pType={pType:电脑}
	 * 返回值：JSONArray
	 * */
...
//			System.out.println("----发布状态监视：res_goods:" + res_goods + "  res_goods_user:" + res_goods_user);
	 ...
	 * 作者：孙铖铖
	 * 日期：2016-12-13
	 * 注释：必须带上Product_Id，区分此图片是给哪个商品的；
	 * 触发方式：http://192.168.194.2:8080/IslandTrading/analysis/uploadImg?Product_Id=1
	 * */
	 ...
	 * 作者：孙铖铖
	 * 日期：2016-12-13
	 * 触发方式：http://192.168.194.2:8080/IslandTrading/analysis/downloadImg?Product_Id=1
	 * */
...
//		renderText("download_ok");
	 ...
	 * 作者：孙铖铖
	 * 日期：2016-12-15
	 * 注释：只需要 User_TakingId 
	 * 参数：mode 验证登陆还是注册
	 * 触发方式：http://localhost:8080/IslandTrading/analysis/registQQ?mode=regisger&User_TakingId=12345&Hx_Username=12345&Hx_Password=123abc
	 * 			http://localhost:8080/IslandTrading/analysis/registQQ?mode=check&User_TakingId=12345
	 * */
	public void registQQ(){
		...
		if(mode.equals("check")){	//登陆
			...
			mRecord = new Record()
					.set("User_Username", User_TakingId)
					.set("User_TakingId", User_TakingId)
					.set("User_Power", 0)
					.set("Hx_Username", Hx_Username_c)
					.set("Hx_Password", Hx_Password_c);
	...
	public void myRelease(){
		...
		list = orderService.myRel(User_Id);
	 ...
	 * 涉及表：re_product_user、islandtrading_product
	 * 运行注释：通过商品状态判断商品是否卖出
	 * 触发方式：http://localhost:8080/IslandTrading/analysis/mySell?User_Id=700	注意，这里没引号
	 * */
	public void mySell(){
		int User_Id = this.getParaToInt("User_Id");
		
		List<Record> list = new ArrayList<>();
		list = orderService.mySell(User_Id);
	...
	public void myBuy(){
		int User_Id = this.getParaToInt("User_Id");
		List<Record> list = new ArrayList<>();
		list = orderService.myBuy(User_Id);
